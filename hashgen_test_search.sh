...
mkdir -p results
filename="./results/results_1.csv"
...
./vault/vaultx -t 32 -i 1 -m 2048 -k 30 -g data-16GB.tmp -f data-16GB.bin
...
for diff in 3 4; do
    ...
    ./vault/vaultx -k 30 -f data-16GB.bin -s 1000 -q $diff -d true 2>&1| \
...
./vault/vaultx -t 32 -i 1 -m 2048 -k 31 -g data-32GB.tmp -f data-32GB.bin
...
for diff in 3 4; do
    ...
    # Run vaultx and collect its metrics
    ./vault/vaultx -k 31 -f data-32GB.bin -s 1000 -q $diff -d true 2>&1| \
    awk -v diffLvl="$diff" -v searches="1000" -v kval="31" -v testtype="$testType" '
    /Actual file size on disk/    { filesize=$7 }
    /Search Summary:/ {
        match($0,/found_queries=([0-9]+)/,m); found=m[1]
        match($0,/notfound=([0-9]+)/,n); notfound=n[1]
...
./vault/vaultx -t 32 -i 1 -m 2048 -k 32 -g data-64GB.tmp -f data-64GB.bin
...
for diff in 3 4; do
    ...
    # Run vaultx and collect its metrics
    ./vault/vaultx -k 32 -f data-64GB.bin -s 1000 -q $diff -d true 2>&1| \
    awk -v diffLvl="$diff" -v searches="1000" -v kval="32" -v testtype="$testType" '
    /Actual file size on disk/    { filesize=$7 }
    /Search Summary:/ {
        match($0,/found_queries=([0-9]+)/,m); found=m[1]
        match($0,/notfound=([0-9]+)/,n); notfound=n[1]
